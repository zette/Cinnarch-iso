...
make_customize_root_image() {
    if [[ ! -e ${work_dir}/build.${FUNCNAME} ]]; then
       ...
       sed -i "s/#Server/Server/g" ${work_dir}/root-image/etc/pacman.d/mirrorlist
        ...
        # sed -i -e "s/^#//g" ${work_dir}/root-image/etc/locale.gen


        mkarchiso ${verbose} -w "${work_dir}" -D "${install_dir}" \
            -r 'glib-compile-schemas /usr/share/glib-2.0/schemas/' \
            run
        ...
        mkarchiso ${verbose} -w "${work_dir}" -C "${pacman_conf}" -D "${install_dir}" \
            -r 'su -c "xdg-user-dirs-update" cinnarch' \
            run

        sed -i 's|^Exec=|Exec=sudo |g' ${work_dir}/root-image/usr/share/applications/pacmanxg4.desktop
        sed -i 's|^pacmanxg|sudo pacmanxg |g' ${work_dir}/root-image/usr/bin/libreoffice-installer
        ...
        mkarchiso ${verbose} -w "${work_dir}" -C "${pacman_conf}" -D "${install_dir}" \
            -r 'systemctl -f enable pacman-init.service lightdm.service NetworkManager.service || true' \
            run
        : > ${work_dir}/build.${FUNCNAME}
    fi
}
